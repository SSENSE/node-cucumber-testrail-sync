      ...
      walk.sync(this.config.stepDefinitionsDir, function (filePath) {
        if (!fs.lstatSync(filePath).isDirectory()) {
          ...
          .map(function (line) {
            ...
            var step = { keyword: matches[1], regexp: matches[2] };
            return step;
          });

          this.implementedSteps.concat(stepDefinitions);
        }
      }.bind(this));
      next();
    }.bind(this),
...
ScenarioSynchronizer.prototype.getTestFileContent = function (gherkinSteps, template) {
  ...
  for (var i = 0; i < gherkinSteps.length; i++) {
    ...
    if (stepDefinitionMatch.length === 0) {
      ...
      var match1 = step.regex.match(NUMBER_MATCHING_GROUP);
      var match2 = step.regex.match(QUOTED_STRING_MATCHING_GROUP);
...
ScenarioSynchronizer.prototype.verifySync = function (callback) {
  ...
  this.testrailClient.getTests(this.plan.entries[0].runs[0].id, function (err, testcases) {
    ...
    async.eachSeries(testcases, function iterate(testcase, nextCase) {
      if (this.testFiles[testcase.case_id] === 'undefined') {
        return nextCase('"' + testcase.title + '" not found locally');
      }
      ...
      if (localFileContent !== remoteFileContent) {
        return nextCase('Local test case "' + testcase.title + '" is outdated');
      }
