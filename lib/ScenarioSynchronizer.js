...
function ScenarioSynchronizer(options, callback) {
  ...
  async.series([
    ...
    function validateTemplateOption(next) {
      if (this.options.stepDefinitionsTemplate) {
        fs.stat(path.resolve(this.templateDir, this.options.stepDefinitionsTemplate + '.hbs'), function statCallback(err, stat) {
          if (err) {
            if (err.code === 'ENOENT') {
              return next(new Error('Validation error: template file ' + this.options.stepDefinitionsTemplate + '.hbs does not exists'));
            }
            return next(err);
          } else if (!stat.isFile()) {
            return next(new Error('Validation error: template file ' + this.options.stepDefinitionsTemplate + '.hbs is not a file'));
          }
          return next();
        }.bind(this));
      } else {
...
ScenarioSynchronizer.prototype.synchronizeCase = function (testcase, relativePath, callback) {
  ...
  var stepDefinitionsExtension = this.options.stepDefinitionsTemplate ? this.getTestFileExtension(this.options.stepDefinitionsTemplate) : '';
  var stepDefinitionsPath = path.resolve(this.options.stepDefinitionsDir + '/' + relativePath, basename + '.' + stepDefinitionsExtension);
  ...
  async.series([
    ...
    function createStepDefinitionsDirectoryStructure(next) {
      mkdirp(this.options.stepDefinitionsDir + '/' + relativePath, next);
    }.bind(this),
    ...
    function synchronizeFiles(next) {
      ...
      if (!exists.feature) {
        ...
        if (!exists.step_definitions && this.options.stepDefinitionsTemplate) {
          fs.writeFileSync(stepDefinitionsPath, this.getTestFileContent(gherkins, this.options.stepDefinitionsTemplate));
        }
