...
ScenarioSynchronizer.prototype.getTestFileContent = function (gherkinsSteps, template) {
  ...
  for (var i = 0; i < gherkinsSteps.length; i++) {
    ...
    if (i+1 < gherkinsSteps.length) {
      if(gherkinsSteps[i+1][0] === '|') { // an example table
        params = 'table, ';
    ...
    var regex = gherkinsWords.join(' ');
    var regex = escapeStringRegexp(regex).replace(/\//g, '\\/');
    ...
    function synchronizeFiles(next) {
      ...
      if (!exists.feature) {
        ...
        if (!exists.step_definitions && this.config.stepDefinitionsTemplate) {
          fs.writeFileSync(stepDefinitionsPath, this.getTestFileContent(gherkins, this.config.stepDefinitionsTemplate));
        }
